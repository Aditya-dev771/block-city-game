...
async function player(label){const email=`${label}-${crypto.randomUUID()}@example.test`;const{data,error}=await admin.auth.admin.createUser({email,password,email_confirm:true,user_metadata:{username:label}});assert.ifError(error);const client=createClient(url,anonKey,{auth:{persistSession:false}});const login=await client.auth.signInWithPassword({email,password});assert.ifError(login.error);return{id:data.user.id,client};}
...
const anonymous=createClient(url,anonKey,{auth:{persistSession:false}});const anonymousOpen=await anonymous.rpc('open_business',{requested_business_type:'restaurant',request_key:crypto.randomUUID()});assert(anonymousOpen.error,'anonymous economy RPC must fail');
