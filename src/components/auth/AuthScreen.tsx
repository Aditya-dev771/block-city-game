...
export function AuthScreen() {
  ...
  return <main className="min-h-dvh bg-ink p-4 text-cream sm:grid sm:place-items-center">
    <div className="mx-auto grid min-h-[calc(100dvh-2rem)] max-w-5xl overflow-hidden rounded-3xl bg-[#254632] shadow-2xl sm:min-h-0 sm:grid-cols-[1.1fr_.9fr]">
      ...
      <section className="flex min-h-full flex-col justify-center bg-cream p-6 text-ink sm:p-10"><div className="mb-8 flex items-center gap-3"><span className="rounded-xl bg-moss p-2 text-cream"><Leaf/></span><div><p className="font-display text-xl font-bold">Founding Era</p><p className="text-xs uppercase tracking-widest text-ink/50">Town economy</p></div></div><h2 className="font-display text-3xl font-bold">{mode === 'login' ? 'Welcome home' : 'Become a founder'}</h2><p className="mt-2 text-sm text-ink/60">{mode === 'login' ? 'Continue your resident’s story.' : 'Your starter home and resident are waiting.'}</p>
